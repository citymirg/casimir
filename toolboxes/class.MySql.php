...
class MySQL {
	...
	function Insert($aVars, $sTable, $aExclude = ''){
        ...
        // Adds a record to the database, or deletes and inserts if primary key 
	// exists . based on the array key names
	function Replace($aVars, $sTable, $aExclude = ''){
		// Catch Exceptions
		if($aExclude == ''){
			$aExclude = array();
		}
		...
		$sSQLQuery = 'REPLACE INTO `' . $sTable . '` SET ';
		foreach($aVars as $iKey=>$sValue){
			if(in_array($iKey, $aExclude)){
				continue;
			}
